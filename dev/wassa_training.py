...
from wassa_utils import cross_correlation_comp, smoothing, kernels_similarity, correlation_kernels, factors_similarity, sparsity_kl, compute_factors_distributions, in_notebook
...
def learn_motifs(model,training_set,testing_set,training_parameters,path,metric_names,sm=None,online_plots=False,verbose=True):
    ...
    if os.path.isfile(path + '.pth'):
        ...
        training_metrics,metric_names = torch.load(path + '_loss.pth',weights_only=True)
    else:
        ...
        if training_parameters['tied_weights']:
        ...
        for i_step in (tqdm(range(training_parameters['N_learnsteps'])) if verbose else range(training_parameters['N_learnsteps'])):
            ...
            with torch.no_grad():
                model.normalization()
                training_metrics[i_step] = compute_training_metrics(metric_names, model, criterion, testing_set, loss, penalty_value, factors_train, sm)
                if online_plots and i_step%10==0:
                    online_monitoring(training_metrics,metric_names,i_step,start_time)
...
def input_normalization(reconstruction,X,training_parameters):
    if training_parameters['normalize_input']:
        if training_parameters['reshape_input']:
            # reshape the dataset in raster plots with the same (temporal) size as the kernels and samples among them
            n_samples = torch.div(X.shape[-1],training_parameters['kernel_size'][-1],rounding_mode='floor')
            true_samples = X[:,:,:n_samples*training_parameters['kernel_size'][-1]].reshape([X.shape[0],X.shape[1],n_samples,training_parameters['kernel_size'][-1]]).swapaxes(1,2).flatten(end_dim=1)
            reconstructed_samples = reconstruction[:,:,:n_samples*training_parameters['kernel_size'][-1]].reshape([X.shape[0],X.shape[1],n_samples,training_parameters['kernel_size'][-1]]).swapaxes(1,2).flatten(end_dim=1)
            sampling = torch.randperm(true_samples.shape[0])[:true_samples.shape[0]//2]
            true_samples = true_samples.div(true_samples.sum(dim=-1, keepdim=True)+1e-14)[sampling]
            reconstructed_samples = reconstructed_samples.div(reconstructed_samples.sum(dim=-1, keepdim=True)+1e-14)[sampling]
        else:
            reconstructed_samples = reconstruction.div(reconstruction.sum(dim=-1, keepdim=True)+1e-14)
            true_samples = X.div(X.sum(dim=-1, keepdim=True)+1e-14)
        return reconstructed_samples, true_samples
    else:
...
def penalty(model,factors,training_parameters):
    ...
    if 'smoothed_orthogonality' in training_parameters['penalty_type'] and training_parameters['smoothwind']>0:
        ...
        penalty += lambda_*factors_similarity(smoothed_factors)
    if 'kernels_orthogonality' in training_parameters['penalty_type']:
        ...
        if model.tied_weights:
            ...
            penalty += lambda_*kernels_similarity(model.encoder[0].weight)
    if 'l1_sparsity_factors' in training_parameters['penalty_type']:
        lambda_ = training_parameters['lambda'][training_parameters['penalty_type'].index('l1_sparsity_factors')]
    ...
    if 'bias' in training_parameters['penalty_type']:
        ...
        if model.do_bias[0]:
            penalty += lambda_*model.bias[0]
        if model.do_bias[2]:
            penalty += lambda_*model.decoder[0].bias.sum()
    return penalty

def compute_training_metrics(metric_names, model, criterion, testing_set, loss, penalty_value, factors_train,sm):
    if model.tied_weights:
        learnt_weights = model.weights.detach()
    else:
        learnt_weights = model.decoder[0].weight.detach()
    ...
    for ind_m, metric in enumerate(metric_names):
        ...
        if metric == 'testing loss':
            loss_ = criterion(testset_hat,testing_set)
        ...
        if metric == 'train/test factors difference':
            trainset_latent_dist, testset_latent_dist = compute_factors_distributions(factors_train, factors_test)
            training_metrics[ind_m] = kl_div(trainset_latent_dist, testset_latent_dist).detach()
        ...
        if metric == 'mean timings similarity':
            ...
            training_metrics[ind_m] = correlation_kernels(mean_timings, learnt_weights)
    return training_metrics

def unsupervised_learning(parameters,dataset,metric_names,dataset_name,k_fold=1,n_iter=1,last_steps=.05,synthetic_metrics=[],record_path='../results/',device='cpu',verbose=False):
        ...
        for k in range(k_fold):
            ...
            if len(parameters)>1:
                ...
                for params in parameters:
                    ...
                    autoencoder, training_metrics, metric_names = learn_motifs(autoencoder,trainset,testset,params,model_name,metric_names,verbose=verbose)
            ...
            if autoencoder.tied_weights:
                ...
                nan_condition = torch.isnan(autoencoder.decoder[0].weight.data).sum()==0
            if nan_condition:
                n_last_steps = int(last_steps*parameters[0]['N_learnsteps'])
                if len(synthetic_metrics)==0:
                    results[i,k,:] = training_metrics[n_last_steps:].mean(axis=0)
                else:
                    similarities_gt = get_similarity(sm, autoencoder, testset, synthetic_metrics)
...
def assertion_check(training_parameters,metric_names,synthetic_metrics):
    ...
    assert len(set(metric_names).intersection(['training loss','testing loss','penalty value','explained variance','train/test factors difference','kernels correlation']))==len(metric_names), 'Check training metrics name'
    assert len(set(training_parameters['penalty_type']).intersection([None,'cc','smoothed_orthogonality','kernels_orthogonality','kl_sparsity_factors','l1_sparsity_factors','bias']))==len(training_parameters['penalty_type']), 'Check penalty name'
    assert len(set(synthetic_metrics).intersection(['factors similarity', 'kernels similarity', 'mean timings similarity', 'mse', 'emd', 'emd mean timings']))==len(synthetic_metrics), 'Check synthetic similarity metrics name'
