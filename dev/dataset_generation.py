...
def get_dataset_parameters(dataset_parameters):

    name = ''
    for key in dataset_parameters.keys():
        if key=='N_involved' or key=='freq_sms' or key=='coefficient_variation':
            name += f'{dataset_parameters[key][0]}'
        elif key!='frs':
...
class sm_generative_model:
    def __init__(self, dataset_parameters, device='cpu'):
        ...
        p_m = min((self.opt['freq_sms']*self.opt['N_delays']*1e-3).sum().item(),1)*torch.ones_like(self.opt['frs'],device=device)
        self.avg_fr_nonmot = self.opt['frs']/(self.opt['coefficient_variation']*p_m+torch.ones_like(self.opt['frs'],device=device)-p_m)
    ...
    def draw_input(self, nb_trials=10):
        ...
        # iterate over the different occurrences of motifs to modify the local distribution
            for k in range(self.opt['N_SMs']):
                ...
                for n in range(nb_motif_k):
                    ...
                    previous = input_proba[trial,:,time-(kernels.shape[-1]//2):time+kernels.shape[-1]//2+1].squeeze(0)
        ...
        input_proba[input_proba==-1] = 1/kernels.shape[-1]
...
def generate_dataset(parameters, record_path='../synthetic_data/', verbose=True,  device='cpu'):
    ...
    if os.path.exists(dataset_path):
        ...
        dataset_input = torch.zeros(parameters['N_samples'],parameters['N_pre'],parameters['N_timesteps'], device=device)
        dataset_output = torch.zeros(parameters['N_samples'],parameters['N_SMs'],parameters['N_timesteps'], device=device)
        dataset_input[dataset_input_list] = 1
        dataset_output[dataset_output_list] = 1
    else:
        dataset_input, dataset_output = sm.draw_input(nb_trials=parameters['N_samples'])
    ...
    return sm, dataset_input, dataset_output
